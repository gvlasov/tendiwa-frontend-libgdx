...
import org.tendiwa.modules.mainModule.ontology.Shout;
...
public class StdActions implements ActionsAdder {
	...
	public void addTo(InputToActionMapper actionMapper) {
		...
		actionMapper.putAction(Modifiers.shift + S, new KeyboardAction("action.shout") {
			@Override
			public void act() {
				volition.actionWithoutTarget(new Shout());
			}
		});
			...
			public void act(int screenX, int screenY) {
				...
				taskManager.trySettingTask(new Task() {
					...
					public boolean ended() {
						return forcedEnd || player.position().equals(cell);
					}
	...
	/**
	 * Is there is an enemy in a cell x:y, then this method will push a request to attack that enemy, otherwise this
	 * method
