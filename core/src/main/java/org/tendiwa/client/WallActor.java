 ...
 */
public final class WallActor extends Actor {
	...
	static {
		...
		for (TextureAtlas.AtlasRegion region : regions) {
			...
			if (true) {
				throw new RuntimeException("did not code getting a wall by name");
			}
			wallHeights.put(
//				Registry.wallTypes.get(region.name),
	...
	/**
	 * Computes a hash describing the image of this wall: which sides the wall faces, which transitions are applied to
	 * it.
...
//	depthTestBatch.setShader(GameScreen.drawWithRGB06Shader);
	...
	/**
	 * For a transition where there is no neighbor from that side, draws an opaque black transition; for a transition
	 * to
	 * an
